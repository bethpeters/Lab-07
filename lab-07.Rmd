...
author: "Fire Emoji - Faaiz, Mohsin, Beth, Beatrice"
date: "`r Sys.Date()`"
...
evals %>%
  summarise(median(score), mean(score))
```

The distribution is skewed towards students positively rating courses. i calculated the mean to be 4.2 and the median to be 4.3, solidfying the view that students are more likley to rate a course positively.
...
Jitter adds small amounts of random variation to the location of each point.
The initial plot shows discrete data whereas scatter plots are meant to have continuous data.
